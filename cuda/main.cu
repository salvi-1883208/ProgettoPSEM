...
#define BLOCKS 12               // number of blocks
...
__global__ void dla_kernel(int* grid, int* skipped, curandState* state) {
  ...
  while (i < MAX_ITER) {
    ...
    if (!(y <= 0 || y >= (gridSize - 1) || x <= 0 ||
          x >= (gridSize -
                1)) &&  // in bounds  (with this condition no particle will ever
                        // get stuck on a border, could be made so that it
                        // doesn't check outside of the grid )
      ...
      // if the particle is close to an already stuck particle, attach it to the
      // grid
    ...
    // while the move is not ok generate a new move TODO do this with a for not
    // a while
    do {
      ...
      i++;
    } while (tempY < 0 || tempY > (gridSize - 1) || tempX < 0 ||
             tempX > (gridSize - 1));
...
int main(void) {
  ...
  cudaMalloc((void**)&d_state,
             blocks * threads_per_block * sizeof(curandState));
  ...
  *skipped = 0;
  ...
  // this implementation ignores the overlapping particles and re-launches them
